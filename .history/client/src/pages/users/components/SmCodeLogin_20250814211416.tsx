...
const SmCodeLogin: React.FC = function ({ form, FormItem, Input }) {
  ...
  const submitUserInfo = async (data: number) => {
    const res: any[] = await getUsers();
    console.log(res);
    const mobileList: any[] = res.data.map((item: any) => item.mobile);
  ...
  return (
    <>
      <FormItem name="mobile" rules={LoginRule.mobileRule} hasFeedback>
        <Input placeholder="请输入手机号" prefix={IconMap.mobile} onChange={checkMobile} onFinish={submitUserInfo} />
      </FormItem>
