...
const Login: React.FC = function () {
  ...
  useEffect(() => {
    // 组件挂载时测试
    message.error("测试错误消息"); // v5 直接用
    // 或 v4：messageApi.error('测试错误消息');
  }, []);
  ...
  const submitUserInfo = async (data: any) => {
    ...
    try {
      ...
      const matchedUser = validUsers.find(
        (user) => user.accountName.trim() === inputAccount && user.password === inputPassword // 注意：加密密码需用bcrypt.compare
      );

      // 4. 打印最终匹配结果（用于调试）
      console.log("输入账号:", inputAccount);
      console.log("输入密码:", inputPassword);
      console.log("是否匹配到用户:", !!matchedUser);
